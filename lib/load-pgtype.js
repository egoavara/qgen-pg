export async function loadPgtypeAllOids(tx, namespace, name) {
...
export async function loadPgtypeByName(tx, namespace, name) {
...
export async function loadPgtypeByOid(tx, oid) {
...
where pc."oid" = $1`, [temp.rows[0].typrelid]);
        ...
        return {
            ...
            fields: Object.fromEntries(await Promise.all(Object.entries(rawFields.rows[0].fields).map(async ([k, v]) => {
                const pgtype = await loadPgtypeByOid(tx, v);
                if (pgtype === undefined) {
...
//# sourceMappingURL=load-pgtype.js.map